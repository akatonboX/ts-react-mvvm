...
import { SourceKey, BindingPath, Invalid, TypedBindingPath, isBindingPath} from "./binding-common";
import { SourceProxy, ReadonlySourceProxy, SourceManager, BindingSource, CollectionSource, NormalBindingSource } from "./binding-source";
import { ValueChangedEventArgs, ChangedValue, EventManager, SourceChangedEventArgs, ErrorChangedEventArgs, InputChangedEventArgs } from "./binding-event";
import { BindingReceiver, isBinding, Binding } from "./binding-receiver";
import { ValidateError, InputOption, Input, InputManager, InputRef } from "./binding-input";
...
function useRender(){
  ...
  return ()=>{
    if(componentState.current)
      setVersion(version === Number.MAX_VALUE ? Number.MIN_VALUE : version + 1);
  };
}
    ...
    const bindingSource = (
      () => {
        if(currentBindingSouce !== undefined){
          ...
          const bindingSource = new NormalBindingSource(sourceKey, source);
...
export function useInput<T_VALUE, T_INPUT_VALUE>(
  ...
  const id = initParameters.current.id;
  ...
  const input: Input<T_INPUT_VALUE> = {
    id: id,
    name: option.name,
    ...
    onInputChange: isEnable ? (inputValue) => {
      ...
      if(input === undefined)
        throw Error(`Input.onInputChange: id not found. id=${String(id)}, name=${option.name}`);
      ...
      if(input.getError().filter(error => !error.allowSet).length === 0){
        ...
        if(bindingSource !== undefined){
          const oldValue = bindingSource.getValue(source.$path);
          const newValue = convertBack(converted, oldValue);
          if(newValue !== Invalid && oldValue !== newValue){
            bindingSource.setValue(source.$path, newValue);
          }
        }
      }
    } : inputValue => {},
    onValidation: isEnable ? () => {
      const input = InputManager.find(id);
      if(input === undefined)
        throw Error(`Input.onValidation: id not found. id=${String(id)}, name=${option.name}`);
      input.validate();
    }: () => {},
  }
  ...
  inputRef.validate = isEnable ? () => {
    const bindingSource = SourceManager.find(source.$sourceKey);
    if(bindingSource !== undefined){
      const currentValue = bindingSource.getValue(source.$path);
...
export function useValidateErrors(target: BindingPath | string | symbol): ValidateError[]{
  ...
  return inputRefs.reduce((previousValue, inputRef) => {
    return [...previousValue, ...inputRef.getError()]
  }, [] as ValidateError[]);
}

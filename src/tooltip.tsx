...
export function Tooltip(
  ...
  const color = props.backgroundColor === undefined ? Tooltip.Default.backgroundColor : props.backgroundColor;
  ...
  const onTargetClick = (event: React.MouseEvent<HTMLSpanElement, MouseEvent>): void => {
    ...
    if (isNull(tooltipElement)) throw Error('');
    if (tooltipElement.style.visibility === 'collapse') {
      ...
      const tooltipStyle = (
        () => {
          ...
          if (tooltipRect.left - idealMarginLeft + tooltipRect.width > window.innerWidth) {
      ...
      : (
        <div ref={myElementRef} className={styles.container}>
          <span
            ...
            style={{
              color: textColor, backgroundColor: color, borderColor: color, visibility: 'collapse',
            }}

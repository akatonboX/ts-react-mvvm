...
import {
  Path, TypedPath, $path, $pathFrom,
} from './path';
import {
  BindingPath, SourceKey, $bindingPath, TypedBindingPath,
} from './binding-common';
...
class MultiBinding<T_SOURCE, T_RESULT> implements Binding<T_RESULT> {
  ...
  getValue(receiver: BindingReceiver): T_RESULT | undefined {
    const prop = Object.keys(this.source).reduce((previouseValue, key) => { previouseValue[key] = this.source[key as keyof T_SOURCE].getValue(receiver); return previouseValue; }, {} as any) as {[K in keyof T_SOURCE]+?: T_SOURCE[K]};
...
export function $bind<T_SOURCE, T_RESULT = T_SOURCE>(source: TypedBindingPath<T_SOURCE>, defaultValueOrConverter?: T_RESULT | ((prop: T_SOURCE) => T_RESULT)): Binding<T_RESULT>;
...
export class BindingReceiver {
  ...
  private addSourceRenderedVersions(sourceKey: SourceKey) {
    if (this.sourceRenderedVersions.find((item) => item.sourceKey === sourceKey) === undefined) {
      const source = SourceManager.find(sourceKey);
      this.sourceRenderedVersions = [...this.sourceRenderedVersions, { sourceKey, version: source === undefined ? undefined : source.version }];
    }
  }
...
function createBindingProxy<T>(receiver: BindingReceiver, sourceKey: SourceKey, path: TypedPath<T>): BindingProxy<T> {
  return new Proxy(new BindingProxyImpl(receiver, sourceKey, path) as any, {
    get: (target: BindingProxyImpl<T>, name: PropertyKey, rec: any): any => {
      if (name in target) {

...
import {
  isFunction, isObject, isArray, isNumber,
} from 'util';
import {
  SourceKey, $bindingPath, BindingPath, TypedBindingPath,
} from './binding-common';
import {
  $path, $pathFrom, Path, TypedPath,
} from './path';
import {
  EventManager, createValueChangedEventArgs, SourceChangedEventArgs, ValueChangedEventArgs,
} from './binding-event';
import { InputManager } from './binding-input';

export class SourceManagerImpl {
  ...
  regist(bindingSource: BindingSource): void{
    if (this.find(bindingSource.sourceKey) !== undefined) throw Error(`Key already exists. sourceKey=${String(bindingSource.sourceKey)}`);
    this.bindingSources.push(bindingSource);
  }

  remove(sourceKey: SourceKey): void{
    const bindingSource = this.find(sourceKey);
    if (bindingSource === undefined) throw new Error(`BindingSourceManager.remove: Key does not exist. sourceKey = ${sourceKey.toString()}`);
...
if (process.env.NODE_ENV === 'development') (window as any).SourceManager = SourceManager;
...
abstract class BindingSourceBase<T_INIT_PARAM> implements BindingSource {
  ...
  update(param: T_INIT_PARAM): void {
    ...
    EventManager.sourceChanged.call(this, { sourceKey: this.sourceKey, oldSource, newSource: this.source });
  }
  ...
  validate() {
    ...
    inputs.forEach((inputRef) => {
      inputRef.setError([]);
      if (inputRef.isEnable) inputRef.validate();
    });
  }
...
export type SourceProxyItem<T> = (
  T extends (infer Z)[] ? SourceProxyItem<Z>[]
  : T extends object ? {
      [K in keyof T]: (
  ...
  get(target: T, name: PropertyKey, receiver: any): any {
    ...
    if (name in target) {
      if (isFunction(childValue)) {
        return (...args: any[]) => Reflect.apply(childValue, currentValue, args);
      }
...
export class CollectionSource<T> extends BindingSourceBase<CollectionSourceParam<T>> {
  ...
  private createSource(
    ...
    const sorted = converted.sort((item1, item2) => {
      ...
      if (item1.sortKey === item2.sortKey) return 0;
      if (item1.sortKey < item2.sortKey) return -1;
      return 1;
    });
    const sortedAndFilter = sorted.filter((item) => item.isVisible);
    const { length } = sortedAndFilter;
    for (let i = 0; i < length; i++) {
      const item = sortedAndFilter[i];
      item.isGroupHeaders = i === 0
        ? item.groups.map((group) => true)
        : item.groups.map((group, index) => sortedAndFilter[i - 1].groups[index] !== group);
    ...
    if (this.bindingPath !== undefined && args.sourceKey === this.bindingPath.$sourceKey) {
      if (this.bindingPath.$path.$equals(args.originalPath)) {
        ...
        if (isNumber(index)) {
          ...
          } else if (currentItem.sortKey !== newItem.sortKey || !currentItem.groups.reduce<boolean>((acc, value, valueIndex) => acc && value === newItem.groups[valueIndex], true)) {
            this.reset();
          } else {
